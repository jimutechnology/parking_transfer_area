...
class GroundNode(ServiceNode):
	def __init__(self):
		...
		param = AreaParam()
		self.setRate(0.1)
		self.Subscriber("scan", LaserScan, self.scan_rx)
		...
		with self.lock_scan_data:
			if self.to_scan_ground:
				...
				if not self.b_lidar1_scanned:
					print data.header.frame_id, self.frame_id_1
				...
				if not self.b_lidar2_scanned:
					print data.header.frame_id, self.frame_id_2
	...
	def process_scans(self):
		...
		for r in self.scan_1.ranges:
			...
			if x > self.max_ground_x or x < self.min_ground_x or y > self.max_ground_y or y < self.min_ground_y:
				return False
